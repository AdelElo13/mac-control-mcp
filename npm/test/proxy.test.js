...
/**
 * Self-signed cert for localhost. openssl ships with macOS; if it is missing
 * or too old for -addext the whole suite skips rather than failing spuriously.
 * @returns {{key: string, cert: string}|null}
 */
function makeSelfSignedCert() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'mcmcp-proxy-tls-'));
